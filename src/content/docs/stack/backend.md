...
The backend (the "PFX platform") is a fleet of **Java 21 / Spring Boot 3.5** services built with Gradle, running on EKS with Istio, speaking REST at the edge and **SQS + Kinesis** for events. For much of the member domain, **ABC Financial is the external system of record** — several services exist to sync with or proxy vendors like ABC and Zuora.
